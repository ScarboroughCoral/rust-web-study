...
impl Handler for PageNotFoundHandler {
    fn handle(_: &HttpRequest) -> HttpResponse {
        HttpResponse::new("400", None, Self::load_file("404.html"))
    }
}
...
impl Handler for StaticPageHandler {
    fn handle(req: &HttpRequest) -> HttpResponse {
        let Resource::Path(resource) = &req.resource;
        let route:Vec<&str> = resource.split("/").collect();
        match route[1] {
            ...
            path => match Self::load_file(path) {
                Some(s) => {
                    ...
                    HttpResponse::new("200", Some(map), Some(s))
                },
                _ => HttpResponse::new("200", None, Self::load_file("404.html")),
            }
        }
    }
}
